import math
import numpy as np
from dpcomp_core.monolithic.ahp import ahp_engine,ahp_fast_engine
...
class ahpND_fast_engine(estimate_engine.estimate_engine):
    ...
    def Run(self, Q, x, epsilon, seed):
        ...
        engine = ahp_fast_engine(self._ratio, self._eta)
        shape = x.shape
        hatx1d = engine.Run(Q, np.reshape(x, x.size), epsilon, seed)  # call AHP on 1d array
        hatx1d = np.array(hatx1d) # might come back as a list
        return np.reshape(hatx1d, shape)
