...
class dawa2D_engine(routine_engine.transform_engine_qtqmatrix_linear, Marshallable):
    ...
    def Run(self, Q, x, epsilon, seed):
        ...
        logging.debug('shape of 1d transform is {0} and type is {1}'.format(x1d.shape, x1d.dtype))

        # STEP 2: similarly transform queries
        m = len(Q)
        Qmat = np.zeros([m, len(x1d)])  # workload matrix: m by n, for m queries over 1d domain of size n
        for i in range(m):
            query = Q[i]
