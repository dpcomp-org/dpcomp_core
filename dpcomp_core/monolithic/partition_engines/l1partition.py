...
def L1partition(x, epsilon, ratio=0.5, gethist=False,seed=None):
    ...
    if gethist:
        ...
        for lb in hist[1:]:
            ...
            if lb == 0:
                break
        logging.info('  L1-PART: number of buckets %s' % str(bucks[:5]) )                   
        return bucks
    else:
        for lb in hist[1:]:
...
def L1partition_approx(x, epsilon, ratio=0.5, gethist=False,seed =None):
    ...
    if gethist:
        ...
        for lb in hist[1:]:
            ...
            if lb == 0:
                break
#       moved this logging to routine_engine 
#       logging.info('  L1-PART: number of buckets %i' % len(bucks) )                   
#       logging.info('  L1-PART: buckets %s' % str(bucks) )                 
        return bucks
    else:
        for lb in hist[1:]:
            hatx[lb:rb] = max(0, sum(x[lb:rb]) + prng.laplace(0, 1.0/(epsilon*(1-ratio)), 1)) / float(rb - lb)
