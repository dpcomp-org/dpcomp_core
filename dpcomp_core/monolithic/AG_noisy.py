...
class AG_engine_Noisy(estimate_engine.estimate_engine):
    """ Estimate a 2D dataset by using adaptive grids. """

    def __init__(self, c = 10, c2 = 5, alpha = 0.5,totalBudget=0.01, short_name = 'AGrid_Noisy'):
    ...
    def Run(self, Q, x, epsilon, seed):
            ...
            cells,counts = AG_engine_Noisy.GenerateCells(x,n,m,num1,num2,grid)

            y = AG_engine_Noisy.CountPerturb(x,cells,counts,epsilon,self.alpha,self.c2, prng)
    ...
    def CountPerturb(x,cells,counts,epsilon,alpha,c2, prng):
        ...
        for k in range(len(cells)):
            ...
            newcells,newcounts = AG_engine_Noisy.GenerateCells(curX,nn,mm,num1,num2,newgrid)
