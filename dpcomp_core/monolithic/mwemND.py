...
class mwemND_simple_engine(estimate_engine.estimate_engine):
    ...
    def _exponentialMechanism(x, hatx, Q, epsilon,prng):
        ...
        logging.info('  EXP-MECH: epsilon  %f' % epsilon )  
        logging.info('  EXP-MECH: OPTIMAL index %s / error %f / query %s' % (error.argmax(), merr, Q[error.argmax()]) )
        
        prob = numpy.exp( epsilon* (error - merr) / 2.0 )

        logging.info('  EXP-MECH: prob sum %f / prob %s' % (sum(prob), sorted(prob,reverse=True)[0:5]))

        sample = prng.random_sample() * sum(prob)
        for c in range(len(prob)):
            sample -= prob[c]
            if sample <= 0:
                logging.info('  EXP-MECH: CHOSEN index %s / error %f / query %s' % (c, error[c], Q[c]) )
                return c
        
        logging.info('  EXP-MECH: CHOSEN index %s / error %f / query %s' % (len(prob)-1, error[len(prob)-1], Q[len(prob)-1]) )
        return len(prob)-1
